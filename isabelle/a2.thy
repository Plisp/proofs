...
1-(d) Prove that adding an element to a list by @{term Cons} always gives a list
  greater than the original list.
\<close>
...
lemma m_add_empty[simp]: "m_add Map.empty m = m"
  by (simp add: ext m_add_def)

lemma m_add_empty2[simp]: "m_add m Map.empty = m"
  by (simp add: ext m_add_def split: option.splits)
...
lemma m_count_x_eq_a:
  "m_count ls x = None \<Longrightarrow> m_count (x # ls) x = Some 0 \<and>
...
lemma m_count_sort_a:
  ...
  unfolding sort'_def
  apply(simp only: foldr.simps insort'.simps)
  apply(simp only: Fun.o_apply)
  by (rule m_count_insort)
...
lemma m_count_remove1:
  ...
  apply(auto)
   ...
   apply(simp)
   apply(erule m_count_remove1_idem)
  apply(rule ext)
...
lemma loadI_eval:
  "(rs, \<sigma>, LoadI r x) \<Down> rs' \<Longrightarrow> rs' = rs(r := x)"
  by (simp add: sem_LoadI sem_det)

lemma load_eval:
  "(rs, \<sigma>, Load r v) \<Down> rs' \<Longrightarrow> rs' = rs(r := \<sigma> v)"
  by (simp add: sem_Load sem_det)
...
lemma compile_no_modify_lower_reg:
  ...
  apply(induct e arbitrary: r r' r'' rs rs' p)
    apply(clarsimp)
    apply(drule loadI_eval, simp)
   apply(clarsimp)
   apply(drule load_eval, simp)
  apply(simp split: prod.splits)
  apply(clarify)
  apply(erule sem.cases, simp_all)
  apply(clarify)
  apply(erule_tac ?a2.0=rs'' in sem.cases, simp_all)
  apply(clarify)
  apply(subgoal_tac "rs''a = rs'b(r := rs'b r + rs'b x2)")
   apply(clarsimp)
   apply(frule compile_reg_increase)
   apply(metis order.strict_trans) (* r'' < r < x2 *)
  apply(erule sem_det)
  apply(rule sem_Add)
  done
...
lemma compile_correct:
  ...
  apply(subgoal_tac "rs''a = rs'b(r := rs'b r + rs'b x2)")
   ...
   apply(frule compile_reg_increase)
   apply(clarsimp)
   apply(erule_tac rs'=rs'b and p=x1a in compile_no_modify_lower_reg)
    apply(auto)
  apply(erule sem_det)
  apply(rule sem_Add)
  done
...
lemma s_sem_Seq:
  "s_sem_n (Suc n) (rs, \<sigma>, p) (rs', \<sigma>, p')
   \<Longrightarrow> \<exists>bs bp. ((rs, \<sigma>, p) \<leadsto> (bs, \<sigma>, bp)) \<and>
               s_sem_n n (bs, \<sigma>, bp) (rs', \<sigma>, p')"
  using s_sem.cases
  by fastforce
...
lemma s_sem_n_Seq:
  ...
  apply(subgoal_tac "\<And>n rs.
       \<lbrakk>\<And>rs p. s_sem_n n (rs, \<sigma>, p) (rs', \<sigma>, p') \<Longrightarrow>
               s_sem_n n (rs, \<sigma>, p ;; q) (rs', \<sigma>, p' ;; q);
        \<exists>bs bp. ((rs, \<sigma>, p) \<leadsto> (bs, \<sigma>, bp)) \<and>
                s_sem_n n (bs, \<sigma>, bp) (rs', \<sigma>, p')\<rbrakk>
       \<Longrightarrow> s_sem_n (Suc n) (rs, \<sigma>, p ;; q) (rs', \<sigma>, p' ;; q)")
...
lemma s_sem_n_correct:
  ...
  apply(induct ms rs' rule: sem.induct)
     apply(simp_all del: Fun.fun_upd_apply)
     apply(rule exI[where x=1])
     apply(simp del: Fun.fun_upd_apply)
     apply(rule_tac s_sem.intros(1))
    apply(metis s_sem.intros(2) s_sem_n.simps) (* etc *)
   apply(metis s_sem.intros(3) s_sem_n.simps)
  apply(clarsimp)
  apply(drule_tac q=q in s_sem_n_Seq)
  apply(cut_tac rs=rs' and p=q and \<sigma>=\<sigma> in s_sem.intros(5))
  using s_sem_n.simps(2) s_sem_n_add
  by fastforce
